'''
Taken from https://huggingface.co/ramsrigouthamg/t5_paraphraser
...
bleurt_threshold = int(sys.argv[1])
initial_top_k, initial_top_p, offset_top_k, offset_top_p = int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5])
...
bleurt_scorer = BleurtScorer('/home/animesh/MIforSE/bleurt-scorebleurt/bleurt/bleurt-base-128/')
...
def get_mi_score(s1, s2): # returns average of s1 and s2
    ...
    outputs = mi_model(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, labels=None)
...
def generate_paraphrases(sentence, top_k, top_p):
    ...
    encoding = paraphrasing_tokenizer.encode_plus(text, pad_to_max_length=True, return_tensors="pt")
...
def write_paraphrases(input_file, output_file, position): # position of sentence in the input tsv
    ...
    with open(input_file, 'r') as f:
        for l in f.readlines()[1:]:
            sentence, paraphrases, bad_sentences, top_k, top_p, c = l.strip().split('\t')[position], [], set(), initial_top_k, initial_top_p, 1
            for p in generate_paraphrases(sentence, top_k, top_p):
                if p not in bad_sentences:
                    if get_mi_score(sentence, paraphrase) and get_bleurt(sentence, paraphrase) < bleurt_threshold:
                        paraphrases.append(p)
                    else:
                        bad_sentences.add(p)
            while paraphrases and c <= 10:
                top_k += offset_top_k
                top_p -= offset_top_p
                for p in generate_paraphrases(sentence, top_k, top_p):
                    if p not in bad_sentences:
