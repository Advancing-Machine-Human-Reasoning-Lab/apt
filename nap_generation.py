...
paraphrasing_model = T5ForConditionalGeneration.from_pretrained(
    "ramsrigouthamg/t5_paraphraser"
).to("cuda:2")
paraphrasing_tokenizer = T5Tokenizer.from_pretrained("ramsrigouthamg/t5_paraphraser")
bleurt_scorer = BleurtScorer(
    "/home/animesh/MIforSE/bleurt-scorebleurt/bleurt/bleurt-base-128/"
)
mi_tokenizer = AutoTokenizer.from_pretrained(
    "ynie/roberta-large-snli_mnli_fever_anli_R1_R2_R3-nli"
)
mi_model = AutoModelForSequenceClassification.from_pretrained(
    "ynie/roberta-large-snli_mnli_fever_anli_R1_R2_R3-nli"
)  # predicts E, N, C


def get_mi_score(s1, s2):  # returns average of s1 and s2
    tokenized_input_seq_pair = mi_tokenizer.encode_plus(
...
def generate_paraphrases(sentence, top_k, top_p):
    ...
    for beam_output in beam_outputs:
        sent = paraphrasing_tokenizer.decode(
...
def write_paraphrases(
    input_file, output_file, position
):  # position of sentence in the input tsv
        ...
        for l in f.readlines()[1:]:
            ...
            for p in generate_paraphrases(sentence, top_k, top_p):
                if p not in bad_sentences:
                    if (
                        get_mi_score(sentence, paraphrase)
                        and get_bleurt(sentence, paraphrase) < bleurt_threshold
                    ):
